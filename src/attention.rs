...
pub fn flash_attention(
    ...
    destruct!([n_q, d_q] = q.shape());
    destruct!([n_o, d_o] = o.shape());
    destruct!([seq_k, d_k] = k.shape());
    destruct!([seq_v, d_v] = v.shape());
...
mod test {
    ...
    fn test_flash_attention() {
        ...
        let v = Tensor::from_dim_slice(types::F64, &[S, D]);
        let o = Tensor::from_dim_slice(types::F64, &[N, D]);

        let q_data = random_data(N * D);
        let k_data = random_data(S * D);
        let v_data = random_data(S * D);
